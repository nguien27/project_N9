...
def chuan_hoa_audio(tin_hieu):
    # Chuan hoa tin hieu audio ve khoang [-1, 1] (max normalization)
...
def nang_cao_tan_so_cao(tin_hieu, he_so=1.5):
    # Pre-emphasis - Nang cao tan so cao
    # Giup nhan ro cac dac trung tan so cao (co quan phoi)    
    tin_hieu_emphasized = np.append(tin_hieu[0], tin_hieu[1:] - he_so * tin_hieu[:-1])
    
    return tin_hieu_emphasized

def tien_xu_ly_audio_tho_nang_cao(tin_hieu, tan_so, tan_so_mau=16000, thoi_luong=4):
    # Tien xu ly tong hop cho tin hieu audio tho - Phien ban nang cao
    # Cac buoc: giam nhieu -> loc bandpass -> pre-emphasis -> chuan hoa -> cat lang
...
def tai_du_lieu_audio_nang_cao(thu_muc_audio, tan_so_mau=16000, thoi_luong_doan=4, su_dung_nang_cao=True):
    # Load tat ca file audio tu thu muc, tien xu ly nang cao, va cat thanh nhieu doan
    du_lieu_file = {}
    thong_ke_cat = {
        'tong_so_file_goc': 0,
        'tong_so_doan_cat': 0,
    ...
    for ten_file in cac_file:
        try:
            ...
            # Su dung tien xu ly nang cao hoac co ban
            if su_dung_nang_cao:
